// build-domains2.js - Generate spells-domains.js using Node.js
// Node can directly evaluate the JS data file

const fs = require('fs');

// Load and parse spells data
const content = fs.readFileSync('js/spells-data.js', 'utf8');
const fn = new Function(content + '; return SPELLS;');
const SPELLS = fn();

console.log(`Loaded ${SPELLS.length} spells from spells-data.js`);
...
SPELLS.forEach(s => {
  ...
  // Also index by the ID itself for debugging
});
...
// Spell names from d20srd.org D&D 3.5e SRD
const DOMAINS = {
...
console.log(`Done: js/spells-domains.js generated`);
console.log(`  Mapped ${Object.keys(spellToDomains).length} spells to domains`);
console.log(`  Total: ${Object.keys(DOMAINS).length} domains`);

if (unmapped.length > 0) {
  console.log(`\nUnmapped spells (${unmapped.length}):`);
