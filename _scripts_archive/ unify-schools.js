...
const SCHOOL_MAP = {
  ...
  // 拼写错误
  'Conjuration': '咒法系',  // 拼写错误，应为Conjuration
  'Enchantment': '惑控系',  // 拼写错误，应为Enchantment
  'Illusion': '幻术系',     // 拼写错误，应为Illusion
...
spells = spells.map(s => {
  if (!s.school) {
    console.log(`⚠️  法术 ${s.nameEn || s.nameZh || s.id} 缺少school字段`);
    unknown++;
    return s;
  }
