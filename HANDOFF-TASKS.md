...
**当前覆盖率：约 95%**（从初始 35-40% → 75-80% → 90% → 95%，含P5-P8全部完成）
...
├── skills/        # 35 个技能 + 共效页面
├── feats/         # 28 个专长独立页 + 总览 + 列表页 + 6个分类合集页
├── equipment/     # 装备总览 + 魔法物品 + 武器详述
├── dm/            # DM工具（6页：intro/角色创建/遭遇构建/宝藏生成/随机表格/XP追踪）
├── domains/       # 领域系统（23页：intro + 22个领域详情页）
├── monsters/      # 怪物总览 + 经典怪物 + 龙类 + 7个类型专题页
...
| 6. 怪物条目补充 | ✅ 完成 | P5扩展7页(200+种怪物) + 3页原有 + 3R扩展60种 |
...
### P5 — 怪物图鉴扩展（✅ 完成，commit f1b75bf）

| 页面 | 行数 | 内容 |
|------|------|------|
| `monsters/humanoids.html` | 813 | 地精族、兽人、爬行类、水生类、怪物人形 |
| `monsters/undead.html` | 700 | 低阶/虚体/木乃伊吸血鬼/巫妖/其他不死 |
| `monsters/outsiders.html` | 948 | 元素(6)/天界(6)/恶魔(8)/魔鬼(6)/尤格罗斯(3)/其他(3) |
| `monsters/magical-beasts.html` | 590 | 低/中/高CR魔法兽、空中魔法兽 |
| `monsters/aberrations-fey.html` | 608 | 异怪（经典/地下城/恐怖）+ 精类（自然/黑暗） |
| `monsters/giants-constructs.html` | 459 | 巨人(6+4变种) + 构装体(4魔像+其他) |
| `monsters/plants-oozes.html` | 614 | 植物(低/中/高CR) + 泥怪(经典/稀有) |

### P6 — 领域系统（✅ 完成，commit 534d694）

| 页面 | 说明 |
|------|------|
| `domains/intro.html` | 22领域总览，彩色卡片网格 |
| `domains/air.html` ~ `domains/water.html` (×22) | 每页含神授力量、1-9级领域法术表、相关神祇 |

同步修复：`js/spells-domains.js` 领域命名统一（破坏→毁灭、守序→秩序），`spells/list.html` 领域卡片渲染bug修复，`spells/search.html` 领域常量同步。

### P7 — DM工具页面（✅ 完成，commit b576078）

| 页面 | 行数 | 内容 |
|------|------|------|
| `dm/intro.html` | 366 | DM工具总览，5个工具卡片导航 |
| `dm/character-creation.html` | 414 | 7步建卡指南、购点计算器、掷骰器 |
| `dm/encounter-builder.html` | 416 | EL计算器、每日遭遇预算 |
| `dm/treasure-generator.html` | 484 | 随机宝藏生成器（含JS） |
| `dm/random-tables.html` | 465 | 天气/遭遇/地下城/NPC/姓名随机表 |
| `dm/xp-tracker.html` | 462 | 交互式经验值追踪器 |

### P8 — 专长分类补全（✅ 完成，commit b576078）

| 页面 | 行数 | 内容 |
|------|------|------|
| `feats/combat.html` | 391 | 29个战斗专长，含短卡片和完整卡片 |
| `feats/general.html` | 321 | 22个通用专长 |
| `feats/metamagic.html` | 233 | 9个超魔专长 + 对比表 + 策略指南 |
| `feats/item.html` | 380 | 9个物品制造专长 + JS制造计算器 |
| `feats/spell-feats.html` | 213 | 10个法术专长 |
| `feats/divine.html` | 289 | 11个神圣专长 + Build指南 |
...
| 优先级 | 任务 | 说明 |
|--------|------|------|
| 低 | 旧专长页侧边栏统一 | 29个旧专长页侧边栏未更新分类链接 |
| 低 | 怪物图鉴继续扩充 | 目前约200种，MM有900+，可按需继续添加 |
| 低 | 图片资源 | 仍为纯文字站，可酌情添加插图 |
| 低 | 导航栏布局统一 | 两种布局仍并存 |
...
3. **P1-P8 已全部完成**，核心内容覆盖率约95%
4. **剩余为可选优化**：旧专长页侧边栏统一、怪物继续扩充、图片资源、导航栏统一
